/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.automation.facade;

import com.automation.service.FacebookAccessTokenService;
import com.automation.service.EventGeneratorService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

/**
 * @author root
 */
@Service
public class EventCollectorFacade {
    
    @Autowired
    EventGeneratorService eventGeneratorService;
    
    @Autowired
    FacebookAccessTokenService facebookAccessTokenService;
    
    public void test(){
      eventGeneratorService.initiateCron();
      //facebookAccessTokenService.getAccessToken();
    }
}
